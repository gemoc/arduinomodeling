...
//	}
	...
	public void deleteUnusedExpressions(Sketch sketch) {
		ImmutableList<Instruction> instructions = ImmutableList.copyOf(getAllInstructions(sketch));
	...
	public EObject getNextInstruction(EObject current) {
		...
		if (current instanceof Instruction) {
			...
			List<Instruction> instructions = getAllInstructions((Sketch)current);
	...
	public List<ModuleGet> getModuleGets(EObject container) {
		...
		if (container instanceof Sketch) {
			List<Instruction> instructions = getAllInstructions((Sketch)container);
			for (Instruction instruction : instructions) {
...
//				+ instruction.getModule().getImage();
	...
	private List<Instruction> getAllInstructions(Sketch sketch){
	...
	private List<Instruction> getAllInstructions(fr.obeo.dsl.arduino.Thread thread){
		return thread.getBlock().getInstructions();
	}
	
	public List<Instruction> getAllInstructions(fr.obeo.dsl.arduino.Block block){
		List<Instruction> instructions = new ArrayList<Instruction>();
